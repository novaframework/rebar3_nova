...
init(State) ->
    ...
    case parse_version(Vsn) of
        ...
        ["git"] ->
            rebar_api:info(
                "Compiling with rebar3 from git - make sure you know what you are doing"
            ),
            lists:foldl(
                fun provider_init/2,
                {ok, State},
        ...
        SomethingElse ->
            rebar_api:abort(
                "Nova needs Rebar > 3.15 to function. Your version is: ~p. Please consider upgrading.",
                [SomethingElse]
            )
    end.
